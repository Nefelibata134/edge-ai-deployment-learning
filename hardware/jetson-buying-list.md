...
- Day60 后项目 3 的 STM32、CAN 收发器和桌面电机执行机构。
- 项目 3 采用分批购买，完整清单见
  `hardware/project3-jetson-stm32-buying-list.md`。
- 完整小车底盘、移动电源、IMU、超声波和激光雷达继续延后，不属于项目 3
  第一版。

理由：

- 先完成两个现有项目和 Day60 就业交付，再把项目 2 的风险事件接到 STM32。
- 项目 3 第一版使用桌面编码电机，避免过早变成机械、电池和底盘调试。
...
- `Jetson Orin Nano Super Developer Kit`
- `SN580 500GB NVMe SSD M.2 2280`
- `1080P USB 免驱 摄像头 UVC`

开发板候选：

| 优先级 | 候选 | 页面价格信息 | 判断 |
| --- | --- | --- | --- |
| 推荐 | LOBOROBOT 京东自营旗舰店：NVIDIA Jetson Orin Nano SUPER Developer Kit 官方原装开发套件 | 约 2889-2899 元，页面显示已售 2000+ | 首选。京东自营、销量高、描述明确“官方原装”。 |
| 推荐 | 创乐博京东自营旗舰店：945-13766-0000-000 官方原装开发板套件 | 页面出现约 2556-3199 元等多个套餐，已售 400+ | 可选。下单时注意选择 8GB 官方原装标配，不要误选摄像头/外壳/ORIN NX 套餐。 |
| 可考虑 | 矽递官方旗舰店：Orin Nano 8G Super 官方开发套件 | 约 2869-3099 元，已售 400+ | 可选。注意区分“官方”“官方+固态”“官方+摄像头”等套餐。 |
| 暂不推荐 | 图为智盒工业级/双网口套件 | 约 3800-4300+ 元 | 偏工业定制，价格高，不适合作为首次学习板。 |
| 暂不推荐 | 亚博智能 4GB / 摄像头进阶套餐 | 约 2569-2709 元 | 有 4GB 选项，容易买错；首购不建议。 |
| 不买 | 771 元左右的 ORIN NANO/NX 载板 | 约 771 元 | 这是载板/配件，不是完整开发套件。 |

开发板推荐结论：

1. 优先买京东自营的 LOBOROBOT 官方原装开发套件，约 2899 元。
2. 如果创乐博官方原装 8GB 标配价格明显更低，也可以考虑，但必须确认不是 ORIN NX、不是 4GB、不是单载板。
3. 不建议为了便宜买 1800-2200 元描述含糊的商品。

SSD 建议：

- 搜索结果中有很多第三方店铺、拆机盘和价格异常的商品。
- 建议只买京东自营或品牌官方店。
- 推荐容量：1TB 优先，500GB 可用但后期 Docker、模型、数据集可能紧张。
- 推荐关键词：`致态 TiPlus5000 1TB 京东自营`、`西部数据 SN580 1TB 京东自营`、`三星 980 1TB 京东自营`、`金士顿 NV3 1TB 京东自营`。
- 不建议买“拆机”“二手”“渠道店超低价”的 SSD。

摄像头建议：

- 首选普通 USB 免驱摄像头，1080p 或 4K 均可。
- 京东看到海康威视京东自营 USB 摄像头约 174 元，销量高，适合作为第一阶段摄像头。
- 如果想更像机器视觉项目，可以选 UVC 工业相机，例如中维奥柯/技彩 1080p UVC 工业相机，约 148-238 元。
- 第一阶段不建议直接买深度相机。

本次推荐下单组合：

- 开发板：LOBOROBOT 京东自营官方原装 Jetson Orin Nano SUPER Developer Kit，约 2899 元。
- SSD：京东自营 1TB NVMe M.2 2280，优先致态/西部数据/三星/金士顿。
- 摄像头：海康威视京东自营 USB 免驱摄像头，或 1080p UVC 工业相机。
- U 盘：64GB 或 128GB 京东自营品牌 U 盘。
- 转接线：DisplayPort 转 HDMI，注意方向是 DP 输出到 HDMI 显示器。
- 网线：六类网线即可。
