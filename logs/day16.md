...
- [x] 用户阅读并解释两个核心脚本。
- [x] 用户完成八道检查题。
- [x] 完成 30 分钟 CSIG 主赛推进或竞赛记录复核。
- [x] 完成 Day16 复盘。

## 实际完成与复盘

2026-07-19，用户确认 Day16 全部完成。

- Day15 已经学习过 Builder、Parser、Config 和 Profile，因此今天没有重复逐行学习相同部分。
- 构建脚本只重点阅读新增的 `build_serialized_network()`、engine 写入、构建计时和报告生成，控制重复复习时间。
- FP16 engine 相比 FP32 体积减少约 47.6%，纯 GPU 计算延迟由 2.040 ms 降至 1.207 ms，实测加速约 1.69x。
- Raw output 在严格容差下并非完全 `allclose`，但 99.96% 以上元素接近；真实图片的检测数量、类别顺序一致，检测级验证通过。
- 828 FPS 只代表 engine 计算段理论能力，不能当作摄像头系统端到端 FPS。
- 单张图验证只能发现明显回归，项目 1 仍需使用完整验证集比较 FP32/FP16 的 mAP。
- CSIG 竞赛没有新增可核实的分数或提交记录，因此日志只确认完成了当日竞赛时段，不虚构结果。
