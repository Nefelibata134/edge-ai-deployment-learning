...
当前两个正式项目共享同一条部署主线，但必须证明不同的工程能力。项目 3 已作为
候选方向记录，专门补充 MCU、RTOS、CAN 与物理控制；是否启动及何时启动由
用户按实际需求决定：
...
3. CAVIAR 公开外部验证集：固定 7 段短视频，为越线、停留和 ROI 入侵分别建立开发片段与
   同机位正样本留出片段，停留事件另加一段零事件负样本。人工观看开发视频确定规则语义和
   区域，已有的逐帧人员框自动生成事件真值；规则冻结后才运行留出片段，计算事件
   Precision、Recall、F1、触发时间/锚点偏差和证据完整率。该结果与 MOT17 跟踪指标分开报告。

文件回放是可复现基线，IMX219 CSI 是板端主要实时输入，RTSP 用于网络输入和断线恢复验证。USB UVC 保留为可选扩展。

CAVIAR 原视频为 MPEG2，项目当前文件源只接受 H.264 MP4，因此先保留并校验原始文件
SHA-256，再在 Jetson 上确定性转码，通过本机 RTSP 按 25 FPS 实时回放。这样不会把
文件格式错误或无节流快速解码造成的丢帧误判成检测/事件质量问题。

2026-09-01 已完成首个 CAVIAR 评估周期的开发集人工复核并冻结规则：`Walk1` 使用双向
有限警戒线，`Browse1` 使用右侧展台 ROI 与 `3.0` 秒停留阈值，
`EnterExitCrossingPaths1front` 使用开放商店入口 ROI。CAVIAR 人工轨迹生成的开发集
预期事件数为 `5/2/2`，共 9 个事件均已对照叠加视频确认；留出集必须继续使用同一配置，
不得根据结果修改几何、运行阈值或匹配标准。

留出语义复核进一步确认 `Browse_WhileWaiting2` 的冻结停留 ROI 位置正确，但全程无人进入，
因此保留为零事件负样本，用于验证误报控制；另在查看结果前加入官方描述为“浏览并阅读一段
时间”的 `Browse2` 作为停留正样本留出片段。两者都沿用同一 ROI 和 `3.0` 秒阈值。

四段留出视频已在查看任何 Jetson 预测前完成语义与真值审核，冻结预期事件数为
`Walk2=3`、`Browse_WhileWaiting2=0`、`Browse2=1`、
`EnterExitCrossingPaths2front=3`，共 7 个。入口事件明确采用“启动占用即告警”：视频启动
时已在 ROI 内的人员经连续两帧确认后触发一次事件。正式程序以语义审计和真值审计两道
门禁阻止未复核留出运行；当前尚未执行 Jetson CAVIAR 推理。
...
当前状态：方向已确定，尚未创建正式仓库；等待用户明确决定启动时间和范围。
...
- Day41：已完成项目 1 模型质量门禁工作。
- 原 Day42-Day53：两个项目增强候选项，不再自动执行。
- 原 Day54-Day60：就业交付候选项，不再自动执行。
- 项目 3：可在用户明确提出后启动，不受 Day 编号约束。

`roadmap/60-day-plan.md` 保留历史进度和候选任务，不是自动执行队列。
