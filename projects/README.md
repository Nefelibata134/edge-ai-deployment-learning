...
3. CAVIAR 公开外部验证集：固定 6 段短视频，为越线、停留和 ROI 入侵分别建立一段
   开发片段与一段同机位留出片段。人工观看开发视频确定规则语义和区域，已有的逐帧人员框
   自动生成事件真值；规则冻结后才运行留出片段，计算事件 Precision、Recall、F1、
   触发时间/锚点偏差和证据完整率。该结果与 MOT17 跟踪指标分开报告。
